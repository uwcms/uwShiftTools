class caloTower:
    ...
    @staticmethod
    def initFromString(str):
        from argparse import ArgumentTypeError as error
        caloType = str[0]
        if caloType not in ['E', 'H']:
            raise error("Malformed caloTower string: %s.  Must specify either E or H" % str)
        tok = str[1:].split(',')
        if len(tok) != 2:
            raise error("Malformed caloTower string: %s.  Example format: E3,10" % str)
        ieta, iphi = map(int, tok)
        if caloType == 'E':
            if ieta < -28 or ieta > 28 or ieta == 0:
                raise error("Malformed caloTower string: %s.  iEta out of bounds (-28,28) excluding 0" % str)
        elif caloType == 'H':
            if ieta < -41 or ieta > 41 or ieta == 0 or abs(ieta) == 29:
                raise error("Malformed caloTower string: %s.  iEta out of bounds (-41,41) excluding -29,0,29" % str)
            if abs(ieta) > 29 and iphi % 2 == 0:
                raise error("Malformed caloTower string: %s.  Impossible iPhi for given iEta" % str)
            if abs(ieta) > 39 and iphi % 4 != 3:
                raise error("Malformed caloTower string: %s.  Impossible iPhi for given iEta" % str)
        if iphi <= 0 or iphi > 72:
            raise error("Malformed caloTower string: %s.  iEta out of bounds (1-72)" % str)
    ...
    def linkMaskId(self):
        ...
        if self._caloType == 'E':
            ...
            if (eta < 40) and (self._iphi % 4 == 1):
                ...
                raise Exception("I should not have gotten here... tower: "+str(self))
